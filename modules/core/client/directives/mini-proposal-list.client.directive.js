...
angular.module('core')
	.directive('miniProposalList', ['$timeout', function ($timeout) {
		return {
			restrict: 'E',
			scope: {
				solutionId: '=',
			...
			templateUrl: 'modules/core/client/views/mini-proposals-list.client.view.html',
				...
				function ($scope, $state, $window, VoteService, SortService, Authentication, SocialshareService, RegionService, SolutionService, ProposalService, $mdDialog) {
					...
					vm.$onInit = function () {
						if(vm.solutionId) {
							...
							ProposalService.list({ solutionId: vm.solutionId })
								.then(proposals => vm.proposals = proposals);
						}
					};
